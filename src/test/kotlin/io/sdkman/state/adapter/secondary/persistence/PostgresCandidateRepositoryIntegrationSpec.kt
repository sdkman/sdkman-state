    ...
    ShouldSpec({
        ...
        should("report a newly registered candidate as created") {
            withCleanDatabase {
                ...
                result.shouldBeRight().shouldBeInstanceOf<CandidateRegistrationResult.Registered>()
            }
        }
        ...
        should("report a re-registered candidate as not created") {
            withCleanDatabase {
                ...
                result.shouldBeRight().shouldBeInstanceOf<CandidateRegistrationResult.Updated>()
            }
        }
        ...
        should("update the name on a re-registration") {
            withCleanDatabase {
                ...
                val candidate = repo.upsert(registrationOf("kotlin", name = "Kotlin Language")).shouldBeRight().candidate
        ...
        should("preserve created_at across a re-registration") {
            withCleanDatabase {
                ...
                val first = repo.upsert(registrationOf("kotlin")).shouldBeRight().candidate

                // when: the same identifier is posted again
                val second = repo.upsert(registrationOf("kotlin", name = "Kotlin Language")).shouldBeRight().candidate
