...
15a. **Concurrent posts to one series serialize.** Two simultaneous publications into the same series must not both survive visible. The retirement statement takes a row-level lock over the series before flipping, so the later transaction observes the earlier one's row and retires it. Under the existing read-committed isolation an unlocked implementation can interleave and leave two visible rows.
